...
class CustomIVFPQ:
    """ Custom implementation of IVF-PQ Index
    ...
    m: number of segments within embedded vectors (for PQ)
    pca_dim: number of dimensions to reduce to using PCA (if > 0)
    estimator: clustering estimator ('KMeans' or 'MiniBatchKMeans')
    ...
    def __init__(self, d, nlist, nprobe, m, pca_dim=None, estimator='KMeans', **estimator_kwargs):
        ...
        self.pca_dim = pca_dim  # Dimensions after PCA (if applied)
        self.m = m  # Number of segments for PQ
        self.k = 512  # Number of clusters per segment for PQ (fixed to 256)
        self.ds = d // m  # Size of each segment

        self.centroids = None  # Centroids of IVF clusters
        self.labels = None  # Labels of data points
        self.codes = None  # PQ codes for data points
        self.global_indices = None  # Indices of data points within clusters
        self.PCA = None  # PCA transformation object

        # Initialize the clustering estimator
        ...
        # Initialize PQ estimators for each segment
        if estimator.lower() == 'kmeans':
        ...
        self.is_trained = False  # Flag to check if the estimator is trained

    def train(self, X):
        """ Train the IVF and PQ parts with the data X. """
        ...
        # Train PQ part within each cluster
        ...
        for i in range(self.nlist):
            ...
            if len(cluster_data) > 0:
                for j in range(self.m):
                    ...
                    if len(X_j) >= self.k:  # Ensure enough samples for KMeans
    ...
    def encode(self, X):
        """ Encode the data X using the trained PQ estimators. """
        X = X.astype(np.float32)  # Ensure the data is of type float32
    ...
    def add(self, X):
        """ Add data X to the index after training. """
    ...
    def compute_asymmetric_distances(self, Y, centroids_indices):
        """ Compute asymmetric distances between query points Y and the aggregated vectors using IVF-PQ. """
        ...
        for query_idx in range(n_queries):
            y = Y[query_idx:query_idx+1]  # Single query point
            ...
            for k in range(total_codes):
                for i in range(self.m):
                    distances[0,k] += distance_table[:, i, X[k, i]]

            all_distances.append(distances)
            all_indices.append(indices)

        return all_distances, all_indices, distance_table, X


    def search(self, Y, k):
        """ Search for the k-nearest neighbors of the query points Y. """
        Y = np.atleast_2d(Y)
        ...
        all_distances, all_global_indices, distance_table, X = self.compute_asymmetric_distances(Y, centroids_to_explore)
        ...
        return result_distances, result_indices, distance_table, X
